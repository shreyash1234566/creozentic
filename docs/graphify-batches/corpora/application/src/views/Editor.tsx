...
export default function Editor() {
  ...
  const hooks = Array.isArray(activePlan?.hooks)
    ...
    : [];
  const visualInserts = Array.isArray(activePlan?.visualInserts)
    ? (activePlan.visualInserts as Array<Record<string, unknown>>)
    : [];
      ...
      {project && tab === "visuals" && (
        <Panel title="B-roll & Graphics">
          ...
          <div className="mt-5 space-y-3">
            {visualInserts.length ? (
              visualInserts.map((insert, index) => {
                const id = String(insert.id ?? index);
                const prompt = String(insert.prompt ?? "Source-footage visual");
                const state = String(insert.approvalState ?? "PENDING");
                const assetSource = String(insert.assetSource ?? "");
                return (
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <div className="font-mono text-[10px] uppercase tracking-[0.12em] text-ink-soft">
                          Insert {index + 1} · {String(insert.sourceStrategy ?? "planned")}
                        </div>
                        <p className="mt-2 text-sm">{prompt}</p>
                      </div>
                      <span className="rounded-full border border-line px-2 py-1 font-mono text-[9px] uppercase">
                        {state}
                      </span>
                    </div>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {assetSource && state !== "APPROVED" && (
                        <Btn
                          variant="line"
                          onClick={() =>
                            run(() =>
                              editorAction(projectId, "visual-inserts/approve", {
                                visualInsertId: id,
                              }),
                            )
                          }
              ...
              <p className="rounded-xl border border-dashed border-line p-4 text-sm text-ink-soft">
                Create an EditPlan to produce visual-insert prompts.
              </p>
            )}
          </div>
          <div className="mt-5 flex flex-wrap gap-2">
            <Btn
              onClick={() => run(() => generateEditorVisualInserts(projectId, { aspectRatio: "9:16" }))}
              disabled={busy || !visualInserts.some((insert) => typeof insert.prompt === "string" && insert.prompt.trim())}
            >
              {busy ? "Generating visuals…" : "Generate still inserts"}
            </Btn>
          </div>
        </Panel>
      )}
      {project && tab === "timeline" && (
        <Panel title="Timeline & Render">
          ...
          <div className="mt-5 flex gap-2">
            <Btn               onClick={() =>
                run(() =>
                  editorAction(projectId, "render", {
                    sourceAssetId: String(sourceAssets[0]?.id ?? ""),
                    durationSec: Number(beats.at(-1)?.endSec ?? 15),
                  }),
                )
              }
              disabled={busy || !sourceAssets.length}
            >
              Queue render
            </Btn>
