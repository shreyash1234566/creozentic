...
class SVCAdapter(BaseTool):
    ...
    class InputSchema(BaseTool.BaseInputSchema):
        reqs: str = Field(
            ...,
            description="User requirements for lyrics adaptation"
        )
    ...
    def execute(self, **kwargs):
        ...
        output_path = os.path.join(os.path.dirname(analysis_path), 'raw_lyrics.txt')
        ...
        output_path = os.path.join(os.path.dirname(analysis_path), 'lyrics.txt')
        with open(output_path, 'w', encoding='utf-8') as f:
        ...
        with open(os.path.join(os.path.dirname(analysis_path), 'script.txt'), 'w', encoding='utf-8') as f:
            f.write(result)
        data['text'] = result
        with open(os.path.join(os.path.dirname(analysis_path), f'{name}_cover.json'), 'w', encoding='utf-8') as f:
            json.dump(data, f, ensure_ascii=False, indent=2)

        return {
            "adapted_lyrics": result
        }
