...
function copyTemplate(destDir) {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    const pkg = {
      name: 'twick-yt-downloader-runtime',
      type: 'module',
      dependencies: {
        '@twick/cloud-yt-downloader': 'latest',
...
async function main() {
  ...
  if (!command || ['-h', '--help', 'help'].includes(command)) {
    console.log(`
Usage: twick-yt-downloader <command> [options]

Commands:
  init [dir]             Scaffold AWS container template into [dir] (default: ./twick-yt-downloader-aws)
  build <image> [dir]    Docker build image from [dir] (default: ./twick-yt-downloader-aws)
  ecr-login <region> <accountId>  Login docker to ECR
  push <image> <region> <accountId>  Push image to ECR (repo must exist)

Examples:
  twick-yt-downloader init
  twick-yt-downloader build my-repo:latest
  twick-yt-downloader ecr-login us-east-1 123456789012
  twick-yt-downloader push my-repo:latest us-east-1 123456789012
`);
    return;
  }

  if (command === 'init') {
    const dir = rest[0] || 'twick-yt-downloader-aws';
    copyTemplate(dir);
  ...
  if (command === 'build') {
    const image = rest[0];
    const dir = rest[1] || 'twick-yt-downloader-aws';
